...
% ploting
        f1=figure(1); nfrow=2; nfcol=1;isub=1;
%         subplot(nfrow,nfcol,isub);isub=isub+1;
%         plotunc(mSCF);hold on;plotmultilines(mSWE/200);hold on;plot(oSCF,'r*');
        subplot(nfrow,nfcol,isub);isub=isub+1;
        plotunc(mSCF(:,25:48));hold on;plot(oSCF(25:48),'r*');
        subplot(nfrow,nfcol,isub);isub=isub+1;
        plotmultilines(mSWE(:,25:48));
%         subplot(nfrow,nfcol,isub);isub=isub+1;
...
hgexport(gcf,[pdfpath 'A1_snow_only_gen3_site' num2str(isiteindex) '_' char(isiteid) '_exp' num2str(iexp) '.pdf'],figure_property); %Set desired file name
clf(f1);
clf(f1,'reset')
