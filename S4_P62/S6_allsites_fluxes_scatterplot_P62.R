...
odir = paste0('/Users/shuangma/RESEARCH/WORKFLOW/JCR_FLUXNET_2021_stage3/',projectname,'/S6_scatterplot/')
...
# for (iexp in 9:13){
  for (iexp in 11){
  pdf_name_list[[1]] <- paste0("S6_all_sites_exp",iexp,"_MCMC",percentilv,".pdf") # option 1
  pdf_name_list[[2]] <- paste0("S6_4yr_sites_exp",iexp,"_MCMC",percentilv,".pdf")
  pdf_name_list[[3]] <- paste0("S6_5yr_sites_exp",iexp,"_MCMC",percentilv,".pdf")
  # pdf_name_list[[1]] <- paste0("S6_1-4m_sites_exp",iexp,".pdf") # option 2
    ...
    for (i in 1){ # change input here # option 2
      ...
      varnames = c("CH4", "ET", "NBE","ABGB", "SCF","EWT","GPP","rhch4_rhco2","rhco2_rh","SM_LY1","SM_LY2",
                   "H2O_LY1","H2O_LY2","an_co2_c_ratio","an_ch4_c_ratio") # P62 and newer has GPP from in situ
      noobs = 7 # if EWT is used as independent dta, =6 if EWT is used in MCMC, P26 and after may use EWT as obs 
      # ----------------------------------------------------------------------------
      lg_mod <- list()
      countsite <- 0
      ...
      pdf(paste0(odir,pdf_name_list[[i]]),width = 12., height = 8.)
      ...
      for (ivar in 1:noobs){
          ...
          # m = lg_mod[[ivar]]$mean
          # m = lg_mod[[ivar]]$v50
          m = lg_mod[[ivar]][[paste0('v',percentilv)]]
      ...
      mtext(paste0("exp=",iexp),side=3,line = -1,outer = T)
      #-------------------------------------
      o = lg_ind[['GRACE_EWT']]
      m = lg_mod[[6]][[paste0('v',percentilv)]]
      sum(is.infinite(m))  # lm() can't deal with inf values
      m[is.infinite(m)] = NA
      sum(is.infinite(m))
      ...
      varindex = which(varnames=="an_ch4_c_ratio")
      hist(lg_mod[[varindex]]$mean,main="dist of an_ch4_c_ratio")
      ...
      varindex = which(varnames=="an_co2_c_ratio")
      hist(lg_mod[[varindex]]$mean,main="dist of an_co2_c_ratio")
      mtext(paste0("med =",round(median(lg_mod[[varindex]]$mean),2)),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(lg_mod[[varindex]]$mean),2)),side = 3, line = -4,outer=F,adj=0.2)
