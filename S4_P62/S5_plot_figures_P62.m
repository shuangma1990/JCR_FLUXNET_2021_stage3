...
%   for iexp=(14:15) %$$
    for iexp=[11] %$$
%    for iexp= [6]
      cbfdir = [wd 'CBF_FILES/' num2str(isiteindex) '_' char(isiteid) 'CBF_exp' num2str(iexp) '.cbf.nc'];
      ...
      cbrdir = [wd 'CBR_FILES/' num2str(isiteindex) '_' char(isiteid) 'CBF_exp' num2str(iexp) '.cbf.cbr'];
...
%       wbmatrix(i,:) = [WB.PAW WB.PUW WB.SWE];
      ...
      disp('assigned mod')
      outdata = {mod.NBE,mod.NBEano,mod.rh_ch4,mod.ABGB,mod.ET,mod.SCF,mod.GPP,     mod.swe,...
          mod.rhch4_rhco2,mod.rhco2_rh,mod.mean.rhch4_rhco2,mod.mean.rhco2_rh,...
          mod.qsurf,mod.runoff,mod.melt,mod.infil,mod.prec,mod.snowfall,...
          mod.EWT,mod.H2O_LY1,mod.H2O_LY2,mod.SM_LY1,mod.SM_LY2,...
          mod.Rh,mod.rh_co2,mod.rh_co2_an,mod.NPP,mod.labC,mod.leafC,mod.woodC,mod.rootC,mod.SOM,...
          mod.an_ch4_c_ratio,mod.an_co2_c_ratio}; % ,mod.SOM,mod.LAI,mod.leafC,mod.rh_co2,mod.rh_ch4,mod.soil_moist,mod.NPP,mod.Ra
      valdata = {obs.NBE.values,obs.NBEano,obs.CH4.values,obs.ABGB.values,obs.ET.values,obs.SCF.values,obs.GPP.values}; % ,obs.SOM.values(1),obs.Mean_LAI.values
      valdata_allobs = {obs_allobs.NBE.values,obs_allobs.NBEano,obs_allobs.CH4.values,obs_allobs.ABGB.values,obs_allobs.ET.values,obs_allobs.SCF.values,obs_allobs.GPP.values}; % ,obs.SOM.values(1),obs.Mean_LAI.values
      varnames= {'NBE','NBEano','CH4','ABGB','ET','SCF','GPP',      'SWE'...
          'rhch4_rhco2','rhco2_rh','mean.rhch4_rhco2','mean.rhco2_rh',...
          'qsurf','runoff','melt','infil','prec','snowfall',...
          'EWT','H2O_LY1','H2O_LY2','SM_LY1','SM_LY2',...
          'Rh','rh_co2','rh_co2_an','NPP','labC','leafC','woodC','rootC','SOM',...
          'an_ch4_c_ratio','an_co2_c_ratio'};
...
%% uncomment this line till end and remove the two end above to resume full functions
%           % save plots to figures folder
%           f1 = figure(1);%ifig=ifig+1;
%           isub=1;
%           disp('right before plotting figure 1')
%           for o=1:length(valdata)
%               nfrow=7; % number of figure rows
%               nfcol=4; % number of figure cols
%               if o==4 || o==6 % this is to compare state at beginning of month
...
%                   y_plot=y;
%               else
%                   RMSE = sqrt(nanmean((y_allobs - yhat).^2));  % Root Mean Squared Error
%                   mdl = fitlm(y_allobs,yhat);
%                   y_plot=y_allobs;
%               end
...
%               
%               subplot(nfrow,nfcol,isub);isub=isub+1;
%               scatter(yhat,y_plot);title(['exp' num2str(iexp) ' ' varnames{o}]);
...
%           
%           subplot(nfrow,nfcol,isub);isub=isub+1;
%           plotunc(mod.rh_co2_an);
%           title(['anaerobic rh co2']);
%           
%           subplot(nfrow,nfcol,isub);isub=isub+1;
...
%           
%           subplot(nfrow,nfcol,isub);isub=isub+1;
%           plotunc(mod.transp1+mod.transp2);
%           title(['transp 1+2 (mm/day)']);
%           
%           %       subplot(nfrow,nfcol,isub);isub=isub+1;
%           %       plotunc(mod.transp2);
