...
coords <- data.frame('nsite'=c(1:nrow(coordinate_csv)),'SITE_ID'=coordinate_csv$SITE_ID,'LAT'=coordinate_csv$LAT,'LON'=coordinate_csv$LON,'TYPE'=coordinate_csv$SITE_CLASSIFICATION)
...
setwd(paste('~/RESEARCH/WORKFLOW/JCR_FLUXNET_2021/',sep=''))
write.csv(coords, file = paste("S2_LATLON2CBF.csv",sep=''),row.names = F)
