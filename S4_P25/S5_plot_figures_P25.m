% shuang_main_GC4by5_Apr26_revJCR_beforehydrochanges_addPEQ_addch4rhOBS
% the initial branch was forgotten, ran on this one again
% rhch4_rhco2 is the ratio at each time step
% mean.rhco2_rh is the ratio of the mean fluxes
nop = 'S4_P25';
...
%%            
      ...
      disp('assigned mod')
      outdata = {mod.NBE,mod.NBEano,mod.rh_ch4,mod.ABGB,mod.ET,mod.SCF,...
          mod.rhch4_rhco2,mod.rhco2_rh,mod.mean.rhch4_rhco2,mod.mean.rhco2_rh,mod.soil_moist,mod.fV,mod.fW,mod.fT,mod.fCH4,mod.fTch4,...
          mod.qsurf,mod.qpaw,mod.qpuw,mod.paw2puw,mod.melt,mod.infil,mod.prec,mod.snowfall,...
          mod.EWT,mod.paw,mod.puw,mod.sm_PAW,mod.sm_PUW,...
      ...
      varnames= {'NBE','NBEano','CH4','ABGB','ET','SCF',...
          'rhch4_rhco2','rhco2_rh','mean.rhch4_rhco2','mean.rhco2_rh',...
          'SM','fV','fW','fT','fCH4','fTch4',...
          'qsurf','qpaw','qpuw','paw2puw','melt','infil','prec','snowfall',...
          'EWT','paw','puw','sm_PAW','sm_PUW',...
...
% save plots to figures folder
      ...
      for o=1:(length(varnames)-28)
          nfrow=6; % number of figure rows
          nfcol=4; % number of figure cols
          yhat = median(outdata{o},1)';
          y = valdata{o};
%           y = y(1:xmax);
          % get stats metric
...
%             ylim([xymin xymax]);xlim([xymin xymax]);
          ...
          plotunc(outdata{o});plot(xvalue,valdata{o},'r-','LineWidth',2);title([varnames{o}]);   
...
figure_property.Width= '15'; % Figure width on canvas
...
% -----------------------------------------------------
      ...
      subplot(nfrow,nfcol,isub);isub=isub+1;      
      hist(1000*CBR.PARS(:,MD.PARAMETER_IDs.PAW_z));xlim([1000*MD.parmin(MD.PARAMETER_IDs.PAW_z) 1000*MD.parmax(MD.PARAMETER_IDs.PAW_z)]);
      title(['PAW_z (converted to mm)']);
      ylimits = ylim; ymax = ylimits(2);
      format shortg
      text(10,ymax*3/4,['median=',num2str(round(median(CBR.PARS(:,MD.PARAMETER_IDs.PAW_z)),2))]);
      text(10,ymax*2/3,['obs=',num2str(round(obs.PEQ_PAW_z.max_value,2))]);

      subplot(nfrow,nfcol,isub);isub=isub+1;      
      hist(CBR.PARS(:,MD.PARAMETER_IDs.PAW_por));xlim([MD.parmin(MD.PARAMETER_IDs.PAW_por) MD.parmax(MD.PARAMETER_IDs.PAW_por)]);
      title(['PAW por (0-1)']);
      ylimits = ylim; ymax = ylimits(2);
      format shortg
      text(10,ymax*3/4,['median=',num2str(round(median(CBR.PARS(:,MD.PARAMETER_IDs.PAW_por)),2))]);
      
%       subplot(nfrow,nfcol,isub);isub=isub+1; 
%       A=1000.*CBR.PARS(:,MD.PARAMETER_IDs.PAW_z).*CBR.PARS(:,MD.PARAMETER_IDs.PAW_por);
%       B=repmat(A,1,size(mod.ABGB,2));
%       plotunc(B);
%       title(['PAW por*PAW depth in mm']); 
      
      subplot(nfrow,nfcol,isub);isub=isub+1;      
      hist(CBR.PARS(:,MD.PARAMETER_IDs.S_fv));
%       xlim([MD.parmin(MD.PARAMETER_IDs.PAW_por) MD.parmax(MD.PARAMETER_IDs.PAW_por)]);
      xlim([1 100])
      title(['S_fv (1-100)']); 
      ylimits = ylim; ymax = ylimits(2);
      format shortg
...
%         likelihood.Properties.VariableNames(1:size(likelihood,2)) = ...
%             {'ABGB', 'CH4','DOM','ET','EWT','GPP','SIF','LAI','NBE','ROFF',...
%             'SCF','Mean_ABGB','Mean_FIR','Mean_GPP','Mean_LAI',...
%             'PEQ_Cefficiency','PEQ_CUE','PEQ_iniSnow','PEQ_iniSOM','PEQ_C3frac','PEQ_PAW_z'};
%         ofilename = [csvpath num2str(isiteindex) '_' char(isiteid) '.xlsx'];
%         writetable(likelihood,ofilename,'Sheet',['exp' num2str(iexp)])
%         temp_names = {'ABGB', 'CH4','DOM','ET','EWT','GPP','SIF','LAI','NBE','ROFF',...
%             'SCF','Mean_Biomass','Mean_Fire','Mean_GPP','Mean_LAI',...
%             'PEQ_Cefficiency','PEQ_CUE','PEQ_iniSnow','PEQ_iniSOM','PEQ_C3frac','PEQ_PAW_z'};
% % save chi2
...
%         t_chi2.Properties.VariableNames(1:size(t_chi2,2)) = ...
%             {'ABGB', 'CH4','DOM','ET','EWT','GPP','SIF','LAI','NBE','ROFF',...
%             'SCF','Mean_ABGB','Mean_FIR','Mean_GPP','Mean_LAI',...
%             'PEQ_Cefficiency','PEQ_CUE','PEQ_iniSnow','PEQ_iniSOM','PEQ_C3frac','PEQ_PAW_z'};
...
% %     savetable.Properties.VariableNames(1:size(savetable,2)) = ...
% %     {'ABGB', 'CH4','DOM','ET','EWT','GPP','SIF','LAI','NBE','ROFF',...
% %     'SCF','Mean_ABGB','Mean_FIR','Mean_GPP','Mean_LAI',...
% %     'PEQ_Cefficiency','PEQ_CUE','PEQ_iniSnow','PEQ_iniSOM','PEQ_C3frac','PEQ_PAW_z'};
