...
for (iexp in c(1:2,6:7)){
  pdf_name_list[[1]] <- paste0("S6_all_sites_exp",iexp,"_MCMCmean.pdf") # option 1
  pdf_name_list[[2]] <- paste0("S6_4yr_sites_exp",iexp,"_MCMCmean.pdf")
  pdf_name_list[[3]] <- paste0("S6_5yr_sites_exp",iexp,"_MCMCmean.pdf")
  # pdf_name_list[[1]] <- paste0("S6_1-4m_sites_exp",iexp,".pdf") # option 2
    ...
    # for (i in 1){ # change input here # option 2
      ...
      # --------------------------   change input   here   -------------------------
      varnames = c("CH4", "ET", "NBE","ABGB", "SCF","rhch4_rhaco2","SM","fV","fW","fT","fCH4","fTch4") # P22 and older
      varnames = c("CH4", "ET", "NBE","ABGB", "SCF","EWT","rhch4_rhco2","mean.rhch4_rhco2","rhco2_rh","mean.rhco2_rh","SM","fV","fW","fT","fCH4","fTch4",
      ...
      x[x==Inf]=NA
      x[x==-Inf]=NA      
      plot(x,type='l',main="monthly ch4/co2")
      xrange = quantile(x, probs = c(0.01,0.99) , na.rm = TRUE)
      x[x<xrange[1]]<-NA
      x[x>xrange[2]]<-NA
      # normal space
      hist(x,xlim=xrange, main=" ch4/co2")
      ...
      # mean of TS rhch4    /    mean of TS rhco2
      ## method 1 to get site averaged ch4/co2 ratio, one value per site
      ## this is site level averaged ch4/co2 ratio, one value per site
      ## I calculate the ratio for each month, per MCMC, took stats from MCMC and take average for monthly averaged value
      lg_mod_mean_ch4_co2 <- aggregate(. ~ sitename, data=lg_mod[[ch4_co2_index]], mean)
      plot(lg_mod_mean_ch4_co2$mean,main="M1 monthly averaged ch4/co2")
      hist(lg_mod_mean_ch4_co2$mean,main="M1 monthly averaged ch4/co2") #,xlim=c(0,1)
      # log space
      x=lg_mod_mean_ch4_co2$mean
      x[x==0]=NA
      if (sum(!is.na(x))==0){
        print(paste0('M1 monthly averaged ch4/co2 are all NA'))
      }else{
        hist(log(x), main="M1 log montly averaged ch4/co2")
        mtext(paste0("med =",round(median(log(x),4))),side = 3, line = -2,outer=F,adj=0.2)
        mtext(paste0("mean =",round(mean(log(x),na.rm=T),4)),side = 3, line = -4,outer=F,adj=0.2)
      }
      # method 2 to get site averaged ch4/co2 ratio, one value per site
      # I take MCMC mean/median of [(mean of TS rhco2)/(mean of TS rh)]
      varindex = which(varnames=="mean.rhch4_rhco2")
      x <- lg_mod[[varindex]]$mean
      # x[x==Inf]=NA
      # x[x==-Inf]=NA      
      # plot(x,type='l',main="monthly ch4/co2")
      # xrange = quantile(x, probs = c(0.01,0.99) , na.rm = TRUE)
      ...
      # normal space
      # hist(x,xlim=xrange, main="ch4/co2")
      hist(x, main="M2 ts mean ch4/co2")
      mtext(paste0("med =",round(median(x,na.rm=T),4)),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(x,na.rm=T),4)),side = 3, line = -4,outer=F,adj=0.2)
      # log space
      hist(log(x), main="M2 ts mean log ch4/co2")
      mtext(paste0("med =",round(median(log(x),4))),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(log(x),na.rm=T),4)),side = 3, line = -4,outer=F,adj=0.2)
      
      varindex = which(varnames=="rhco2_rh")
      x <- lg_mod[[varindex]]$mean
      x[x==Inf]=NA
      x[x==-Inf]=NA      
      xrange = quantile(x, probs = c(0.01,0.99) , na.rm = TRUE)
      ...
      x[x>xrange[2]]<-NA
      # normal space
      hist(x,xlim=xrange, main="rhco2_rh")
      ...
      # mean of TS rhco2    /    mean of TS rh
      varindex = which(varnames=="mean.rhco2_rh")
      x <- lg_mod[[varindex]]$mean
      hist(x, main="ts mean rhco2_rh")
      mtext(paste0("med =",round(median(x,na.rm=T),4)),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(x,na.rm=T),4)),side = 3, line = -4,outer=F,adj=0.2)
      # log space
      hist(log(x), main="ts log mean.rhco2_rh")
      mtext(paste0("med =",round(median(log(x),4))),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(log(x),na.rm=T),4)),side = 3, line = -4,outer=F,adj=0.2)
      
      
      hist(lg_df$S_fv,main="S_fv (1-100)")
      ...
      hist(log(lg_df$S_fv), main="log S_fv")
      ...
      hist(lg_df$r_ch4,main="r_ch4 (1e-3-0.9)")
      ...
      hist(log(lg_df$r_ch4), main="log r_ch4")
      ...
      hist(lg_df$Q10rhco2,main="Q10rhco2")
      ...
      hist(log(lg_df$Q10rhco2), main="log Q10rhco2")
      # mtext(paste0("mean =",round(mean(log(lg_mod[[ch4_co2_index]]$mean),2))),side = 3, line = -4,outer=F,adj=0.2)
      ...
      hist(log(lg_df$Q10ch4), main="log Q10ch4")
      # mtext(paste0("mean =",round(mean(log(lg_mod[[ch4_co2_index]]$mean),2))),side = 3, line = -4,outer=F,adj=0.2)
      ...
      # plot(lg_mod[[smindex]]$mean, lg_mod[[CH4_index]]$mean, col=alpha("red",0.1),xlim=c(0,0.8))
      hist(lg_mod[[varindex]]$mean,main="dist of sm_PAW")
      mtext(paste0("med =",round(median(lg_mod[[varindex]]$mean),2)),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(lg_mod[[varindex]]$mean),2)),side = 3, line = -4,outer=F,adj=0.2)
      
      varindex = which(varnames=="sm_PUW")
      # plot(lg_mod[[smindex]]$mean, lg_mod[[CH4_index]]$mean, col=alpha("red",0.1),xlim=c(0,0.8))
      hist(lg_mod[[varindex]]$mean,main="dist of sm_PUW")
      mtext(paste0("med =",round(median(lg_mod[[varindex]]$mean),2)),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(lg_mod[[varindex]]$mean),2)),side = 3, line = -4,outer=F,adj=0.2)
      
      hist(lg_df$PAW_z,main="PAW_z")
      ...
      hist(log(lg_df$PAW_z), main="log PAW_z")
      ...
      hist(lg_df$PAW_por,main="PAW_por")
      ...
      hist(log(lg_df$PAW_por), main="log PAW_por")
      ...
      hist(lg_df$max_infil,main="max_infil")
      ...
      hist(log(lg_df$max_infil), main="log max_infil")
      ...
      hist(lg_df$field_cap,main="field_cap")
      ...
      hist(log(lg_df$field_cap), main="log field_cap")
      ...
      hist(lg_df$moisture,main="moisture")
      ...
      hist(log(lg_df$moisture), main="log moisture")
      ...
      hist(lg_df$thetas_opt,main="thetas_opt")
      ...
      hist(log(lg_df$thetas_opt), main="log thetas_opt")
      ...
      hist(log(lg_df$fwc), main="log fwc")
