...
# for (iexp in 1:6){
  for (iexp in 6){
  pdf_name_list[[1]] <- paste0("S6_all_sites_exp",iexp,"_MCMCmedian.pdf") # option 1
  pdf_name_list[[2]] <- paste0("S6_4yr_sites_exp",iexp,"_MCMCmedian.pdf")
  pdf_name_list[[3]] <- paste0("S6_5yr_sites_exp",iexp,"_MCMCmedian.pdf")
  # pdf_name_list[[1]] <- paste0("S6_1-4m_sites_exp",iexp,".pdf") # option 2
    ...
    # for (i in 1){ # change input here # option 2
      ...
      varnames = c("CH4", "ET", "NBE","ABGB", "SCF","EWT","ch4_co2","rhco2_rh","SM_LY1","SM_LY2",
      ...
      # plot(lg_mod[[smindex]]$v50, lg_mod[[CH4_index]]$v50, col=alpha("red",0.1),xlim=c(0,0.8))
      ...
      # data_inf <- log(lg_mod[[ch4_co2_index]]$v50)
      ...
      hist(lg_mod[[varindex]]$v50,main="dist of H2O_LY1")
      ...
      hist(lg_mod[[varindex]]$v50,main="dist of H2O_LY2")
      mtext(paste0("med =",round(median(lg_mod[[varindex]]$v50),2)),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(lg_mod[[varindex]]$v50),2)),side = 3, line = -4,outer=F,adj=0.2)
      varindex = which(varnames=="SM_LY1")
      hist(lg_mod[[varindex]]$v50,main="dist of SM_LY1")
      mtext(paste0("med =",round(median(lg_mod[[varindex]]$v50),2)),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(lg_mod[[varindex]]$v50),2)),side = 3, line = -4,outer=F,adj=0.2)
      varindex = which(varnames=="SM_LY2")
      hist(lg_mod[[varindex]]$v50,main="dist of SM_LY2")
      mtext(paste0("med =",round(median(lg_mod[[varindex]]$v50),2)),side = 3, line = -2,outer=F,adj=0.2)
      mtext(paste0("mean =",round(mean(lg_mod[[varindex]]$v50),2)),side = 3, line = -4,outer=F,adj=0.2)
