...
for (iexp in c(1:2,6:7)){
  pdf_name_list[[1]] <- paste0("S6_all_sites_exp",iexp,"MCMCmedian.pdf") # option 1
  pdf_name_list[[2]] <- paste0("S6_4yr_sites_exp",iexp,"MCMCmedian.pdf")
  pdf_name_list[[3]] <- paste0("S6_5yr_sites_exp",iexp,"MCMCmedian.pdf")
  # pdf_name_list[[1]] <- paste0("S6_1-4m_sites_exp",iexp,".pdf") # option 2
    ...
    # for (i in 1){ # change input here # option 2
      ...
      x[x==0]=NA
      if (sum(!is.na(x))==0){
        print(paste0('ch4/co2 all values are NA'))
      }else{
      ...
      plot(lg_mod_mean_ch4_co2$v50,main="M1 monthly averaged ch4/co2")
      hist(lg_mod_mean_ch4_co2$v50,main="M1 monthly averaged ch4/co2") #,xlim=c(0,1)
      # log space
      x=lg_mod_mean_ch4_co2$v50
      x[x==0]=NA
      if (sum(!is.na(x))==0){
        print(paste0('monthly averaged ch4/co2 are all NA'))
      }else{
        hist(log(x), main="log montly averaged ch4/co2")
        mtext(paste0("med =",round(median(log(x),4))),side = 3, line = -2,outer=F,adj=0.2)
        mtext(paste0("mean =",round(mean(log(x),na.rm=T),4)),side = 3, line = -4,outer=F,adj=0.2)
      }
