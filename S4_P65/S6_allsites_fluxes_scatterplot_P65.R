  ...
  # pdf_name_list[[1]] <- paste0("S6_1-4m_sites_exp",iexp,".pdf") # option 2
    ...
    # for (i in 1){ # change input here # option 2
      ...
      varnames = c("CH4", "ET", "NBE","ABGB", "SCF","EWT","GPP","rhch4_rhco2","rhco2_rh","SM_LY1","SM_LY2",
                   "H2O_LY1","H2O_LY2") # P62 and newer has GPP from in situ
      ...
      for (ivar in 1:noobs){
          ...
          plot(o,m,main=varnames[ivar], col=alpha("black",0.1),
               xlim=range(o,quantile(m,prob=probv,na.rm=T),na.rm=T),
               ylim=range(o,quantile(m,prob=probv,na.rm=T),na.rm=T))
          # plot(o,m,main=varnames[ivar], col=alpha("black",0.1))
